"""
TTS Voice Quality Evaluation via Gnani ASR — Bengali
Transcribes audio from TTS voice directories using Gnani Prisma v2.5,
then computes WER & CER against Bengali reference sentences.
...
def evaluate_voice(
    ...
    print(f"  Language  : Bengali ({LANGUAGE_CODE})")
    ...
    if not wav_files:
        wav_files = sorted(voice_dir.glob("bengali_benchmark_*.wav"))
    if not wav_files:
        wav_files = sorted(voice_dir.glob("speech_bn-IN_*.wav"))
    if not wav_files:
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate TTS voice quality via Gnani ASR on Bengali benchmark"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Path to Bengali reference JSON file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_dir", default="./bengali_eval_results",
        help="Directory to save results (default: ./bengali_eval_results)"
    )
    ...
    references = [entry["bengali_sentence"] for entry in ref_data]
